...
- ``node`` (required): path to a group or array, either relative to the
  referencing **node** (array or group) or absolute from the store root.
...
2. If ``node`` begins with ``/``, resolve as an absolute path from the
   store root.
3. Otherwise, resolve as a relative path from the **referencing node
   itself** (not its containing group), interpreting ``..`` as parent
   traversal per RFC 3986. The referencing node path is supplied via
   the ``referencing_path`` parameter; if absent, the containing group
...
def resolve_json_pointer(document: dict, pointer: str) -> Any:
    ...
    ----------
    document : dict
        The root document to resolve against.
    pointer : str
        A fully-qualified JSON Pointer starting with ``/``.
    ...
    ------
    XGroupPathError
        If the pointer is malformed or the path does not exist.
    """
        ...
        if isinstance(current, dict):
            ...
            try:
                ...
                raise XGroupPathError(
                    ...
                    attribute="attribute",
                    target_path=pointer,
                    reason=_("List index {idx} out of range.").format(idx=idx),
                )
        else:
            raise XGroupPathError(
                source_path="<unknown>",
...
def _resolve_relative_path(base_path: str, relative: str) -> str:
    """
    Resolve a relative node path against a base path.

    Parameters
    ----------
    base_path : str
        Absolute path of the referencing node (array or group), e.g.
        ``"/gr/day/ssp245/r1i1p1f1/tasmax"``. Use ``"/"`` or ``""``
        for the root.
    relative : str
        Relative path from the base node, e.g. ``"../../../../coords/lat"``.

    Returns
    -------
    str
        Resolved absolute path, e.g. ``"/gr/coords/lat"``.
    """
...
def _get_node(store_root: zarr.Group, abs_path: str, source_path: str, attribute: str):
    """Retrieve a zarr node at an absolute path within a store."""
    key = abs_path.lstrip("/")
    try:
        if key == "" or key == "/":
            return store_root
...
def resolve_ref(
    ...
    *,
    ...
    referencing_path: str | None = None,
    ...
    referencing_group : zarr.Group
        The group containing the referencing array. Used as a fallback
        base for relative path resolution when ``referencing_path`` is
        not supplied.
    store_root : zarr.Group
        Root group of the current store.
    referencing_path : str, optional
        Absolute path of the referencing node (array or group) within
        the store. When supplied, relative ``node`` paths are resolved
        from this path rather than from ``referencing_group.path``.
        Per the ``ref`` convention specification, relative paths are
        relative to the referencing **node itself**, not its containing
        group — so the array path should be passed here, not the group
        path.
    source_path : str
    ...
    ------
    XGroupPathError
        If ``node`` is missing, the path is malformed, or a JSON Pointer
        cannot be resolved.
    ...
    if uri is not None:
        ...
        # Relative path — resolve from the referencing node itself.
        # Use referencing_path if supplied, fall back to the group path.
        target_store = store_root
        base_path = referencing_path or referencing_group.path or "/"
